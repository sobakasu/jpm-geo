...
module Jpm
  module Geo
    ...
    class Point
      ...
      class << self
        def from_lonlat(lonlat, radius: Jpm::Geo.radius)
          from_degrees(lon: lonlat.lon, lat: lonlat.lat, radius: radius)
        end

        def from_degrees(lon:, lat:, radius: Jpm::Geo.radius)
          new(lon: lon, lat: lat, radius: radius, radians: false)
        end

        def from_radians(lon:, lat:, radius: Jpm::Geo.radius, validate: true)
      ...
      def distance_to(point)
        raise ArgumentError, "Jpm::Geo::Point expected" unless point.is_a?(Jpm::Geo::Point)
      ...
      def to_s
        units = radians ? " rad" : "°"
        "Jpm::Geo::Point(#{lat}#{units}, #{lon}#{units})"
      end

      def ==(other)
        return false unless other.is_a?(Jpm::Geo::Point)
      ...
      def initialize(lon:, lat:, radians: false, radius: Jpm::Geo.radius, validate: true)
